...
-- Creo el esquema de coches:

CREATE SCHEMA coches  


-- Creo todas las tablas necesarias:
...
CREATE TABLE coches.revisiones(
...
-- Voy modificando las tablas para crear las claves foráneas entre ellas:

	-- Tabla seguro
ALTER TABLE coches.seguro ADD CONSTRAINT matricula_fk FOREIGN KEY (matricula) REFERENCES coches.coche(matricula);
ALTER TABLE coches.seguro ADD CONSTRAINT id_aseguradora_fk FOREIGN KEY (id_aseguradora) REFERENCES coches.aseguradora(id_aseguradora);

	-- Tabla revisiones
ALTER TABLE coches.revisiones ADD CONSTRAINT matricula_fk FOREIGN KEY (matricula) REFERENCES coches.coche(matricula);
ALTER TABLE coches.revisiones ADD CONSTRAINT id_moneda_fk FOREIGN KEY (id_moneda) REFERENCES coches.moneda(id_moneda);

	-- Tabla coche
ALTER TABLE coches.coche ADD CONSTRAINT id_grupo_fk FOREIGN KEY (id_grupo) REFERENCES coches.grupo(id_grupo);
ALTER TABLE coches.coche ADD CONSTRAINT id_marca_fk FOREIGN KEY (id_marca) REFERENCES coches.marca(id_marca);
ALTER TABLE coches.coche ADD CONSTRAINT id_modelo_fk FOREIGN KEY (id_modelo) REFERENCES coches.modelo(id_modelo);
ALTER TABLE coches.coche ADD CONSTRAINT id_color_fk FOREIGN KEY (id_color) REFERENCES coches.color(id_color);


-- Cargo el set de datos:

CREATE TABLE coches.coches_bruto (
...
INSERT INTO coches.coches_bruto (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- Voy insertando los datos desde la tabla en bruto hacia mi modelo de datos:
 
	-- aseguradora
INSERT INTO coches.aseguradora (aseguradora) 
SELECT aseguradora 
FROM coches.coches_bruto 
GROUP BY aseguradora
ORDER BY aseguradora;

	-- grupo
INSERT INTO coches.grupo (grupo) 
SELECT grupo 
FROM coches.coches_bruto 
GROUP BY grupo
ORDER BY grupo;
	...
	-- marca
INSERT INTO coches.marca (marca) 
SELECT marca 
FROM coches.coches_bruto 
GROUP BY marca
ORDER BY marca;
	...
	-- modelo
INSERT INTO coches.modelo (modelo) 
SELECT modelo 
FROM coches.coches_bruto 
GROUP BY modelo
ORDER BY modelo;
	...
	-- color
INSERT INTO coches.color (color) 
SELECT color 
FROM coches.coches_bruto 
GROUP BY color
ORDER BY color;
	...
	-- coche
INSERT INTO coches.coche (matricula, fecha_compra, kms_totales, id_grupo, id_marca, id_modelo, id_color) 
SELECT matricula, fecha_compra, kms_totales, grupo.id_grupo, marca.id_marca, modelo.id_modelo, color.id_color
FROM coches.coches_bruto
INNER JOIN coches.grupo ON coches.grupo.grupo = coches.coches_bruto.grupo
INNER JOIN coches.marca ON coches.marca.marca = coches.coches_bruto.marca
INNER JOIN coches.modelo ON coches.modelo.modelo = coches.coches_bruto.modelo 
INNER JOIN coches.color ON coches.color.color = coches.coches_bruto.color 
GROUP BY matricula, fecha_compra, kms_totales, grupo.id_grupo, marca.id_marca, modelo.id_modelo, color.id_color
ORDER BY fecha_compra;

	-- moneda
INSERT INTO coches.moneda (moneda) 
SELECT moneda 
FROM coches.coches_bruto 
GROUP BY moneda
ORDER BY moneda;
	...
	-- seguro
INSERT INTO coches.seguro (n_poliza, matricula, id_aseguradora, fecha_alta_seguro) 
SELECT coches_bruto.n_poliza, coche.matricula, aseguradora.id_aseguradora, coches_bruto.fecha_alta_seguro
FROM coches.coches_bruto
INNER JOIN coches.coche on coches.coche.matricula = coches.coches_bruto.matricula
INNER JOIN coches.aseguradora on coches.aseguradora.aseguradora = coches.coches_bruto.aseguradora
GROUP BY n_poliza, coche.matricula, id_aseguradora, fecha_alta_seguro
ORDER BY n_poliza;

	-- revisiones
INSERT INTO coches.revisiones (matricula, fecha_revision, kms_revision, importe_revision, id_moneda)
SELECT coche.matricula, coches_bruto.fecha_revision, coches_bruto.kms_revision, coches_bruto.importe_revision, moneda.id_moneda
FROM coches.coches_bruto 
INNER JOIN coches.coche on coches.coche.matricula = coches.coches_bruto.matricula 
INNER JOIN coches.moneda on coches.moneda.moneda = coches.coches_bruto.moneda 
GROUP BY coche.matricula, fecha_revision, kms_revision, importe_revision, id_moneda
ORDER BY fecha_revision;


-- Ahora ya no necesito la tabla del set de datos original. Procedo a borrarla:

DROP TABLE coches.coches_bruto;


-- Realizo la consulta con los datos requeridos:

SELECT modelo.modelo, marca.marca, grupo.grupo, coche.fecha_compra, coche.matricula, color.color, coche.kms_totales, aseguradora.aseguradora, seguro.n_poliza
FROM coches.coche
INNER JOIN coches.modelo on coches.coche.id_modelo = coches.modelo.id_modelo
INNER JOIN coches.marca on coches.coche.id_marca = coches.marca.id_marca
INNER JOIN coches.grupo on coches.coche.id_grupo = coches.grupo.id_grupo
INNER JOIN coches.color on coches.coche.id_color = coches.color.id_color
INNER JOIN coches.seguro on coches.coche.matricula = coches.seguro.matricula
INNER JOIN coches.aseguradora on coches.seguro.id_aseguradora = coches.aseguradora.id_aseguradora
ORDER BY modelo;
